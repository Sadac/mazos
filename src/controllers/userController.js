...
module.exports.deleteMedal = async (req, res) => {
  const { email, medallaId } = req.body;

  try {
    if (!email || !medallaId) {
      throw createError(400, "EMAIL y MEDALLAID son obligatorios");
    }
    const userExist = await find("Usuarios", "email", email);
    if (userExist[1].rowCount === 0) {
    ...
    const medallaGanada = await sequelize.query(
      `SELECT * FROM "UsuarioMedallas" WHERE
      "usuarioId" = '${userExist[0][0].id}' and "medallaId" = '${medallaId}'`
    );
    if (medallaGanada[1].rowCount === 0) {
      throw createError(
        400,
    ...
    WHERE "usuarioId" = '${userExist[0][0].id}' and "medallaId" = '${medallaId}'
    `);

    res
      .status(200)
      .send({ message: "El usuario ha perdido la medalla exitosamente." });
  } catch (error) {
    console.log(error);
    res.status(error.status).send(error);
  }
